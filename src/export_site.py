...
def _write_index_html(path: Path) -> None:
    # Minimal, dependency-light static page with Leaflet via CDN.
    html = f"""<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
      ...
      function render(events) {{
        ...
        for (const e of filtered) {{
          ...
          if (typeof e.lat === 'number' && typeof e.lon === 'number') {{
            ...
            const popup = `<strong>${{(e.title || '').replaceAll('<','&lt;')}}</strong><br/>` +
                          `${{(e.weekday || '').replaceAll('<','&lt;')}} ${{(e.time || '').replaceAll('<','&lt;')}}<br/>` +
                          `${{(e.location || '').replaceAll('<','&lt;')}}<br/>` +
